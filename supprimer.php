	...
	<?php
		if (isset($_POST['supprimer'])) {
			$news = $pdo->query('SELECT * FROM clients')->fetchAll();
				foreach ($news as $row) {
					echo '<p name="nom" value="'.$row['prenom'].'">'.$row['prenom']." ".$row['nom'].'</p>';
				}
		}
	?>
...
<!-- 	<?php $news = $pdo->query('SELECT * FROM chambres')->fetchAll();
		foreach ($news as $row) {
			echo '<p name="prenom" value="'.$row['numero'].'">'.$row['numero'].'</p>';
		}
	?>

	<?php $news = $pdo->query('SELECT * FROM reservations')->fetchAll();
		foreach ($news as $row) {
			echo '<p name="du" value="'.$row['dateEntree'].'">du : '.$row['dateEntree'].'</p>';
			echo '<p name="au" value="'.$row['dateSortie'].'">au : '.$row['dateSortie'].'</p>';
		}
	?> -->
